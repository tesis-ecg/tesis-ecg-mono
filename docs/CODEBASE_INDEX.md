...
**Migrations** (`back/alembic/versions/`): single migration `4940f777d181_initial_schema.py` — autogenerated from current models, applied fresh (prior 001-004 chain was collapsed when the DB was wiped pre-deploy). To apply: `alembic upgrade head`.
...
**Auth/login**: `front` Login form → `POST /auth/login` (email+password) → `auth_service` calls Auth0 ROPG (`auth0_client.authenticate_user`) → backend issues own JWT (`security.create_access_token`) set as httpOnly `session` cookie (`samesite="lax"` — required for cross-origin frontend/backend on separate Vercel deployments) → subsequent requests authenticated via `get_current_user` (cookie) → `GET /auth/me` returns `UserOut`. The FE never talks to Auth0 directly ("Auth0 mediado por backend").

**User provisioning**: on first login, if no `user` row exists for the Auth0 subject, `auth_service` auto-creates one (`role=medico`) **and** inserts a `doctor` row. `POST /auth/register` (admin-only) does the same: creates in Auth0 via Management API, then inserts `user` + `doctor` (for `medico` role) atomically.
